      ...
      <main class="content px-3 py-4">
        <div class="container">
          ...
          <?php if (isset($_GET['status']) && $_GET['status'] == 'phonealreadyexist') : ?>
            <div class="alert alert-danger alert-dismissible fade show" role="alert">
              Pelanggan gagal ditambahkan, kontak sudah terdaftar!
              <button type="button" class="btn-close" data-bs-dismiss="alert" aria-label="Close"></button>
            </div>
          <?php endif; ?>
          <?php if (isset($_GET['status']) && $_GET['status'] == 'namealreadyexist') : ?>
            <div class="alert alert-danger alert-dismissible fade show" role="alert">
              Pelanggan gagal ditambahkan, nama sudah terdaftar!
              <button type="button" class="btn-close" data-bs-dismiss="alert" aria-label="Close"></button>
            </div>
          <?php endif; ?>
          ...
          <?php if (isset($_GET['status']) && $_GET['status'] == 'editfailed') : ?>
            <div class="alert alert-danger alert-dismissible fade show" role="alert">
              Pelanggan gagal diubah!
              <button type="button" class="btn-close" data-bs-dismiss="alert" aria-label="Close"></button>
            </div>
          <?php endif; ?>
          <div class="h3 fw-bold">Pelanggan</div>
          <hr />
          <div class="table-responsive pt-2">
            <div class="d-flex justify-content-between mb-3">
              ...
              <div class="modal fade" id="exampleModal" tabindex="-1" aria-labelledby="exampleModalLabel" aria-hidden="true">
                <form action="../service/createCustomer.php" method="POST">
                        ...
                        <div class="mb-3">
                          <div class="row">
                            ...
                            <div class="col-6 pb-3">
                              ...
                              <input type="text" name="name" class="form-control" id="exampleFormControlInput1" placeholder="Nama Pelanggan..." required />
                            </div>
                            ...
                            <div class="col-6 pb-3">
                              <label for="exampleDataList" class="form-label">Nomor Handphone</label>
                              <input class="form-control" name="phone" id="exampleDataList" type="number" placeholder="6252XXX" required />
                            </div>
                            <div class="mb-3">
            ...
            <table class="table table-striped table-light text-center">
              ...
              <tbody>
                ...
                $search_query = $search ?
                  "WHERE is_deleted = false AND (name LIKE '%$search%' OR phone_number LIKE '%$search%' OR address LIKE '%$search%')" :
                  'WHERE is_deleted = false';

                // Get total number of results
                $data = mysqli_query($db, "SELECT * FROM customer $search_query");
                $jumlah_data = mysqli_num_rows($data);
                $total_halaman = ceil($jumlah_data / $batas);

                // Get paginated results
                $queryCustomer = mysqli_query($db, "SELECT * FROM customer $search_query ORDER BY name LIMIT $halaman_awal, $batas");
                $nomor = $halaman_awal + 1;
                $no = $halaman_awal + 1;
                while ($customer = mysqli_fetch_array($queryCustomer)) {
                  ...
                  echo "<td class='text-start'>" . $customer['address'] . "</td>";
                  echo "<td class='col-2'>";
                  echo "<a class='btn me-2 btn-warning' href='formEditCustomer.php?id=" . $customer['id'] . "' role='button'><span><i class='lni lni-pencil'></i></span></a>";
                  echo "<a class='btn btn-danger delete-button' href='#' data-id='" . $customer['id'] . "' onclick='confirmDelete(" . $customer['id'] . ")'><span><i class='lni lni-trash-can'></i></span></a>";
                  echo "</td>";
                  echo "</tr>";
                }
                ?>
  ...
  <script>
    function confirmDelete(id) {
      ...
      if (confirmation) {
        ...
        window.location.href = "../service/deleteCustomer.php?id=" + id;
      }
    }
